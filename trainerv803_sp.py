...
# * 800: Combines radiomic and deep learning
# * 801: Supports multiple radiomic csv and yaml files
# * 802: Minor changes, some variables are renamed
# * 803_sp: [Special edition] Loading a special type of model where DL is frozen, pretrained DL is being used.
...
import cv2
from dataloader.radiomic_loader001 import MyDataset
import torch
...
df_train = df_train.sort_values(by='Base names').reset_index(drop=True)
df_test = df_test.sort_values(by='Base names').reset_index(drop=True)
...
# ### Read radiomic dataframes
# radiomic_df_train = pd.read_csv(config.radiomic.train_dir)
# radiomic_df_test = pd.read_csv(config.radiomic.test_dir)

# # Read radiomic feature names from yaml file
# with open(config.radiomic.yaml_dir, 'r') as f:
#     radiomic_feature_names = yaml.safe_load(f)
# radiomic_feature_names = radiomic_feature_names[config.radiomic.feature_type]
# radiomic_feature_names = radiomic_feature_names[:config.model.radiomic_dims[0]] # keep features based-on input_dims (radiomic_dims[0])

# # ### Sort dataframes by name
# df_train = df_train.sort_values(by='Base names').reset_index(drop=True)
# df_test = df_test.sort_values(by='Base names').reset_index(drop=True)
# radiomic_df_train = radiomic_df_train.sort_values(by='Base names').reset_index(drop=True)
# radiomic_df_test = radiomic_df_test.sort_values(by='Base names').reset_index(drop=True)


# ### Read radiomic dataframes and radiomic feature names
radiomic_dfs_train = []
radiomic_dfs_test = []
radiomic_feature_names_list = []

for radiomic_train_dir, radiomic_test_dir, yaml_dir in zip(config.radiomic.train_dir, 
                                         config.radiomic.test_dir, 
                                         config.radiomic.yaml_dir):
    # Read CSV files
    radiomic_df_train = pd.read_csv(radiomic_train_dir)
    radiomic_df_test = pd.read_csv(radiomic_test_dir)
    
    # Sort dataframes by 'Base names'
    radiomic_df_train = radiomic_df_train.sort_values(by='Base names').reset_index(drop=True)
    radiomic_df_test = radiomic_df_test.sort_values(by='Base names').reset_index(drop=True)
    
    # Append to respective lists
    radiomic_dfs_train.append(radiomic_df_train)
    radiomic_dfs_test.append(radiomic_df_test)
    
    # Read radiomic feature names from yaml file
    with open(yaml_dir, 'r') as f:
        radiomic_feature_names = yaml.safe_load(f)
    
    # Select feature names based on type and dimensions
    radiomic_feature_names = radiomic_feature_names[config.radiomic.feature_type]
    # radiomic_feature_names = radiomic_feature_names[:config.model.radiomic_dims[0]]
    
    # Append feature names to the list
    radiomic_feature_names_list.append(radiomic_feature_names)
...
# ### Train
pretrained_dir = [
    '/research/m324371/Project/adnexal/results/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-23/checkpoints/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-37/best_model.pth',
    '/research/m324371/Project/adnexal/results/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-23/checkpoints/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-58-11/best_model.pth',
    '/research/m324371/Project/adnexal/results/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-23/checkpoints/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_18-21-46/best_model.pth',
    '/research/m324371/Project/adnexal/results/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-23/checkpoints/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_18-47-41/best_model.pth',
    '/research/m324371/Project/adnexal/results/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_17-31-23/checkpoints/EnsembleResNet18Ft512_EfficientNetB2SFt1408_2024-10-25_19-15-38/best_model.pth'
]
...
if config.phase == "train" or config.phase == "both":
    ...
    def train_one_epoch(epoch_index):
        ...
        for i, (inputs, labels, _) in enumerate(train_loader): # >>>>>>>>>>>>> training loader returns input, label, and image name 
            ...
            #     # For loss
            #     avg_loss = running_loss / show_report # loss per batch
    ...
    def train_one_epoch_multimodel(epoch_index):
        running_loss = 0.
        valid_batches = 0
        train_gt, train_pred, train_prob = [], [], []
        ...
        # iter(training_loader) so that we can track the batch
        # index and do some intra-epoch reporting
        for i, (inputs, radiomics, labels, _) in enumerate(train_loader): # >>>>>>>>>>>>> training loader returns input, radiomic, label, and image name 
            # Every data instance is an input + label pair
            ...
            # Make predictions for this batch
            outputs = model(inputs, radiomics)
            ...
            if ONE_HOT:
                ...
                pred = (prob > 0.5).float().cpu().numpy().tolist()
                lbls = labels.cpu().numpy().tolist()
                prob_ = prob.detach().clone().cpu().numpy().tolist() # detach and create a cloned tensor so that it does not affect prob
            ...
            if i % show_report == show_report-1:
                # For loss
                print("*"*40, "Entering to show report")
                avg_loss = running_loss / show_report # loss per batch
                # print('  batch {} loss: {}'.format(i + 1, last_loss))
                tb_x = epoch_index * len(train_loader) + i + 1
                tb_writer.add_scalar('Loss/train', avg_loss, tb_x)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_names, train_class)):
        ...
        kfold_df_train = kfold_df_train.sort_values(by='Base names').reset_index(drop=True) # sort by name
        kfold_df_train.reset_index(drop=True, inplace=True) # Reset index. Start from 0
        
        kfold_df_val = df_train.iloc[val_index]
        kfold_df_val = kfold_df_val.sort_values(by='Base names').reset_index(drop=True) # sort by name
        kfold_df_val.reset_index(drop=True, inplace=True)

        kfold_radiomic_dfs_train, kfold_radiomic_dfs_val = [], []
        for radiomic_df_train in radiomic_dfs_train: 
            kfold_radiomic_df_train = radiomic_df_train.iloc[train_index]
            kfold_radiomic_df_train = kfold_radiomic_df_train.sort_values(by='Base names').reset_index(drop=True)
            kfold_radiomic_dfs_train.append(kfold_radiomic_df_train)
            kfold_radiomic_df_val = radiomic_df_train.iloc[val_index]
            kfold_radiomic_df_val = kfold_radiomic_df_val.sort_values(by='Base names').reset_index(drop=True)
            kfold_radiomic_dfs_val.append(kfold_radiomic_df_val)
        ...
        params["pretrained_dir"] = pretrained_dir[fold] # get pretrained directory for the current round
        ...
        train_dataset = MyDataset(
            ...
            concat=CONCAT,
            radiomic_feature_names=radiomic_feature_names_list,
            radiomic_dataframe=kfold_radiomic_dfs_train,
            )
        ...
        val_dataset = MyDataset(
            ...
            concat=CONCAT,
            radiomic_feature_names=radiomic_feature_names_list,
            radiomic_dataframe=kfold_radiomic_dfs_val,
            )
        ...
        for epoch in range(initial_epoch, EPOCHS):
            ...
            avg_loss, train_accuracy, train_precision, train_recall, train_f1, train_auc = train_one_epoch_multimodel(epoch) ############### turned off writer
            ...
            with torch.no_grad():
              ...
              for i, (vinputs, vradiomics, vlabels, vnames) in enumerate(val_loader):
        
                  # Move to GPU
                  vinputs = vinputs.to(DEVICE)
                  vradiomics = vradiomics.to(DEVICE)
                  vlabels = vlabels.to(DEVICE)
        
                  voutputs = model(vinputs, vradiomics) # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< pay attention to merge
...
if config.phase == "both" or config.phase == "test":
    ...
    test_dataset = MyDataset(
            ...
            concat=CONCAT,
            radiomic_feature_names=radiomic_feature_names_list,
            radiomic_dataframe=radiomic_dfs_test,
            )
    ...
    if config.test.type == "average" or config.test.type == "both":
        ...
        with torch.no_grad():
            for i, (tinputs, tradiomics, tlabels, tnames) in tqdm(enumerate(test_loader)):
        
                # Move to GPU
                tinputs = tinputs.to(DEVICE)
                tradiomics = tradiomics.to(DEVICE)
                ...
                for model in trained_models:
                    model.eval()
                    toutputs_ = model(tinputs, tradiomics)
                ...
                if ONE_HOT:
                    ...
                    pred_class = (test_prob > 0.5).float().data.cpu().numpy().squeeze()
    ...
    if config.test.type == "best" or config.test.type == "both":
        ...
        with torch.no_grad():
            for i, (tinputs, tradiomics, tlabels, tnames) in tqdm(enumerate(test_loader)):
                ...
                toutputs = model(tinputs, tradiomics) # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< pay attention for multimodal
