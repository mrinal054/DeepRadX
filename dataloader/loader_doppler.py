...
from preprocessing_v3 import preprocess
    ...
    def __getitem__(self, index):
        ...
        if self.concat is not None:
            ...
            for key in self.concat: # list images defined by self.concat
                ...
                else: raise ValueError(f"Key '{key}' not found in the dictionary. Possible keys are: image, adnexal, fluid, solid, dopper, and mask.")
            
            im = np.stack(list_imgs, axis=-1) 

        else: 
            im = doppler.copy()  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< previously image
